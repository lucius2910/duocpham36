(function() {
    ...
    var jhiAlertError = {
        template: '<div class="alerts" ng-cloak="" role="alert">' +
                        '<div ng-repeat="alert in $ctrl.alerts" ng-class="[alert.position, {\'toast\': alert.toast}]">' +
                            '<uib-alert ng-cloak="" type="{{alert.type}}" close="alert.close($ctrl.alerts)"><div>{{ alert.msg }}</div></uib-alert>' +
                        '</div>' +
                  '</div>',
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('footballBeApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                ...
                if (angular.isString(errorHeader)) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('footballBeApp.' + fieldError.objectName + '.' + convertedField);
